  ...
  static async connectToWallet(
    ...
    return firstValueFrom(
      fnPipe(
        ...
        catchError((error) =>
          throwError(() => {
            logger?.error({ error }, "Wallet connect failed");
            // Preserve the underlying error message — for the embedded wallet
            // these are user-actionable (PRF unsupported, passkey cancelled).
            return error instanceof Error ? error : new Error("Unable to connect to the wallet.");
          }),
        ),
