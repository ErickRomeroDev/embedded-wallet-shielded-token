...
export interface WalletContext {
  ...
  recheckProofServer: () => Promise<boolean>;
}
...
export const WalletContext = createContext<WalletContext>({
  ...
  disconnect: () => {},
  refresh: async () => {},
  recheckProofServer: async () => false,
});
...
export const useWalletStore = (logger?: Logger): WalletContext => {
  ...
  const refresh = useCallback(async () => {
    ...
    // refresh() is async — without awaiting it the fields below read stale values.
  ...
  // On-demand proof-server probe: proofServerOnline is otherwise only set at
  // connect/refresh time, so a user who starts the proof server later needs a
  // way to re-check without reconnecting the wallet.
  const recheckProofServer = useCallback(async (): Promise<boolean> => {
    const online = await checkProofServerStatus(serviceUriConfig?.proverServerUri);
    setProofServerOnline(online);
    return online;
