...
describe('API', () => {
  ...
  it('should deploy the contract and increment the counter [@slow]', async () => {
    const modularContractDeployed = await api.deploy(providers, { privateCounter: 0 });
    expect(modularContractDeployed).not.toBeNull();

    const counter = await api.displayCounterValue(providers, modularContractDeployed);
    expect(counter.counterValue).toEqual(BigInt(0));

    await new Promise((resolve) => setTimeout(resolve, 2000));
    const response = await api.increment(modularContractDeployed);
    ...
    const counterAfter = await api.displayCounterValue(providers, modularContractDeployed);
