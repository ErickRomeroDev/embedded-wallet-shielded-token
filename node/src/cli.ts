...
const joinContract = async (providers: ModularProviders, rli: Interface): Promise<DeployedModularContract> => {
...
const deployOrJoin = async (
  ...
  rli: Interface,
): Promise<DeployedModularContract | null> => {
  while (true) {
...
export const run = async (config: Config, _logger: Logger, dockerEnv?: DockerComposeEnvironment): Promise<void> => {
  ...
  try {
    ...
    if (dockerEnv !== undefined) {
      ...
      if (config instanceof UndeployedConfig) {
        config.indexer = mapContainerPort(env, config.indexer, 'modular-indexer');
        config.indexerWS = mapContainerPort(env, config.indexerWS, 'modular-indexer');
        config.node = mapContainerPort(env, config.node, 'modular-node');
        config.proofServer = mapContainerPort(env, config.proofServer, 'modular-proof-server');
      }
    }
